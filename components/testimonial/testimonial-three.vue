<template>
  <div class="container-fluid" style="background-image: url(/images/new-img/about/book.png);background-size: cover;">
        <div class="tp-port-2-content-wrap container">
            <div class="row">
              <div class="col-xl-5 book-text">
                  <div class="tp-port-2-title-box">
                    <h2 class="tp-port-2-title-sm mb-35">
                      
                      
                      
                      {{ $t("Book") }}


                    </h2>
                    <div class="tp-port-2-text d-xs-none">
                        <p class="pb-30">
                          {{ $t("BookDesc") }}                        </p>
                        
                    </div>
                  
                  </div>
              </div>
              <div class="col-xl-7 d-flex align-items-center book-btn">
                  <div class="tp-port-2-content"  dir="ltr">
                    <div class="tp-port-2-text ">
                        <p class="pb-30">
                          <nuxt-link class="tp-btn-blue" href="/contact-us#contact-form">


                          <span class="text">
                            
                            
                            {{ $t("BookNow") }}


                          </span>
                          <span class="icon">
                            <svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M1 1L10 10" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M10 1V10H1" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/> </svg>
                          </span>
                      </nuxt-link>


                        </p>
                        
                    </div>
                    
                  </div>
              </div>
            </div>
        </div>
      </div>
</template>

<script setup lang="ts">

</script>